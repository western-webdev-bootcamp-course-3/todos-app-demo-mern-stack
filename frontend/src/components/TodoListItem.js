...
const TodoListItem = ({ index, todo, todos, setTodos }) => {
  ...
  const deleteTodoServer = async (id) => {
    const response = await axios.delete(process.env.REACT_APP_SERVER_URI + `/todos/${id}`);
  ...
  const handleToggle = async () => {
    ...
    if (todo.completed) {
      // uncheck
      response = await updateTodoStatusServer(todo._id, false);
    } else {
      // check
      response = await updateTodoStatusServer(todo._id, true);
    }
  ...
  const updateTodoStatusServer = async (id, newStatus) => {
    const response = axios.patch(process.env.REACT_APP_SERVER_URI + `/todos/${id}`, {
      completed: newStatus,
    });
    return response;
  };
